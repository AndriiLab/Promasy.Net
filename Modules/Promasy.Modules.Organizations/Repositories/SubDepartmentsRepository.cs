...
internal class SubDepartmentsRepository : ISubDepartmentRules, ISubDepartmentsRepository
{
    ...
    public SubDepartmentsRepository(IDatabase database)
    {
        _database = database;
    }
    ...
    {
        return _database.SubDepartments.AsNoTracking().AnyAsync(s => s.Id == id && s.OrganizationId == userOrganizationId, ct);
    }

    public Task<bool> IsSameDepartmentAsync(int id, int userDepartmentId, CancellationToken ct)
    {
        return _database.SubDepartments.AsNoTracking().AnyAsync(s => s.Id == id && s.DepartmentId == userDepartmentId, ct);

    }

    public Task<bool> IsSameSubDepartmentAsync(int id, int userSubDepartmentId, CancellationToken ct)
    {
        return Task.FromResult(id == userSubDepartmentId);
    }
